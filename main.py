...
def main():
    """
    Entry point for the project. Parses command-line arguments and runs the selected step.

    Parameters
    None directly; arguments are provided via the command line.

    Processing steps
    1. Parse command-line arguments to determine which step to run (clean, train, predict).
    2. If 'clean', run data cleaning on the raw dataset and save to TRAIN_FILE.
    3. If 'train', train the selected model.
    4. If 'predict', run prediction using the selected model and input CSV.

    Returns
    None
        The function executes the requested step and prints results to the console.
    """
    ...
    parser.add_argument(
        "--step",
        choices=["clean", "train", "predict"],   # Allowed options
        default="train",                         # Default step is training
    ...
    parser.add_argument(
        "--model",
        type=str,
        default=DEFAULT_MODEL,                   # Default model defined in config.py
    ...
    if args.step == "clean":
        raw_path = DATA_DIR / "HeartAttackData.csv"   # Path to raw dataset
        clean_data(raw_path, TRAIN_FILE)             # Clean and save dataset
    ...
    # If step is 'predict', run prediction using the selected model and input CSV
        ...
        if not args.input:
            # Raise error if no input file is provided
